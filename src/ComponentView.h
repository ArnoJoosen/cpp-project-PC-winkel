...
class ComponentView {
public:
    using value_type = std::weak_ptr<class ComponentBase>;
    using const_iterator = std::vector<std::weak_ptr<class ComponentBase>>::const_iterator;
    ...
    void filter(const std::function<bool(const std::weak_ptr<class ComponentBase>&)>& filter);
    ...
    inline std::weak_ptr<ComponentBase> operator[](int index) { return my_components[index]; }

    void printView();

private:

    std::vector<std::weak_ptr<class ComponentBase>> my_components;
